...
namespace Ceres
{
    ...
    AssetPtr<FontAtlas> ContentManager::LoadFont(std::string fontName, unsigned int size)
    {
        _loadedFonts.emplace_back(FontAtlas(fontName, 2048, size));
        return AssetPtr<FontAtlas>(_loadedFonts, _loadedFonts.size() - 1);
    }
